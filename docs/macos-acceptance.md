...
CI 用本地签名生成测试应用，没有 Apple Developer ID 证书和公证凭证。正式分发前还须验证从浏览器下载后的 Gatekeeper 流程；取得发行证书后可配置 Tauri 的 Apple 签名与公证环境变量，不在仓库中保存证书或口令。

助手自更新仅读取本项目 GitHub Release。目前没有已发布的 Mac 版本时，“没有适合此平台的更新包”应如实显示；完整成功替换测试需要匹配架构、版本和 SHA256 的实际候选包，不通过伪造成功结果代替验收。
